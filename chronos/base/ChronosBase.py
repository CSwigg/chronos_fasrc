...
from chronos.isochrone.MIST import MIST
...
class ChronosBase:
    def __init__(self, data, models='parsec', use_grp=False, isochrone_dirs=None, **kwargs):
        ...
        if ('baraffe' in models.lower()) or ('bhac' in models.lower()):
            ...
            self.fitting_kwargs['fit_range'] = (-2, 12)
        elif 'mist' in models.lower():
            mist_dir = None
            if isochrone_dirs is not None:
                mist_dir = isochrone_dirs.get("mist")
            if mist_dir is None:
                mist_dir = data_path / 'mist_files'
