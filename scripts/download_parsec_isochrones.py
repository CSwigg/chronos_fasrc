...
DEFAULT_PHOTSYS_VERSION = "odfnew"
...
def _age_segments(
    *,
    ...
    return [
        ...
        AgeSegment(
            name="young_linear_1to300myr_1myr",
            is_log_age=False,
        ...
        AgeSegment(
            name=f"old_log_after300myr_{log_step_dex:g}dex",
            is_log_age=True,
            low=float(log_low),
...
def _metallicity_groups() -> list[MetallicityGroup]:
    return [
        MetallicityGroup("mh_m1p5_to_m1p1", -1.5, -1.1, 0.2, 3),
        MetallicityGroup("mh_m0p9", -0.9, -0.9, 0.0, 1),
        MetallicityGroup("mh_m0p5_to_p0p1", -0.5, 0.1, 0.2, 4),
        MetallicityGroup("mh_p0p3", 0.3, 0.3, 0.0, 1),
    ]
    ...
    *,
    ...
    for segment in age_segments:
        for group in metallicity_groups:
            if segment.count * group.count <= max_isochrones_per_request:
                requests.append((segment, group))
                continue
            if group.step <= 0:
                raise ValueError(
                    f"Cannot split metallicity group {group.name}; "
                    f"{segment.count * group.count} isochrones exceeds {max_isochrones_per_request}."
                )
            metals_per_request = max(1, max_isochrones_per_request // max(1, segment.count))
            ...
            for start in range(0, len(values), metals_per_request):
                requests.append((segment, _group_from_values(values[start : start + metals_per_request])))
    return requests
...
def main() -> None:
    ...
    parser.add_argument("--linear-start-yr", type=float, default=1.0e6)
    parser.add_argument("--linear-end-yr", type=float, default=3.0e8)
    parser.add_argument("--linear-step-yr", type=float, default=1.0e6)
